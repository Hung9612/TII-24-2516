function newT= MatchandCalVW_best4_num(amat,vec,Noninx,reinx,rel)
%MatchandCalVW_best4
...
%     vecWinx3=ceil(realinx_W(t)/16);
%     inx_J=realinx_W(t)-16*(vecWinx3-1);
%     relative_J=Noninx.J==inx_J;

    
    vecWinx4=ceil(realinx_W(t)/16);
    inx_H=realinx_W(t)-16*(vecWinx4-1);
...
curinx2avec=(inx_Vjk+4-j)/4;%将有用的relative索引映射到avec的relative
% n_vec=length(reinx.CEGIKM);%计算vec的长度
n_vec=length(reinx.CEG);%计算vec的长度
...
%    realinx2vec=reinx.CEGIK(curinx2vec);%将vec的相对地址映射为real地址
%    inx_vec2=ceil(realinx2vec/16);
%    inx_K=inx_vec1-16*(inx_vec2-1);
%    relative_K=Noninx.K==inx_K;

%     realinx2vec=reinx.CEGI(curinx2vec);%将vec的相对地址映射为real地址
    
%    inx_vec3=ceil(realinx2vec/16);
%    inx_I=realinx2vec-16*(inx_vec3-1);
%    relative_I=Noninx.I==inx_I;

    realinx2vec=reinx.CEG(curinx2vec);%将vec的相对地址映射为real地址
    
   inx_vec4=ceil(realinx2vec/16);
   inx_G=realinx2vec-16*(inx_vec4-1);
   relative_G=Noninx.G==inx_G;
...
%    tempVi_in_avec=as*cs*es*gs*is;
   tempVi_in_avec=as*cs*es*gs;
