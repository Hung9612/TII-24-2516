function newT= MatchandCalVW_best3_num(amat,vec,Noninx,reinx,rel)
newmat=[];
subnewVmat{16,1}=[];
subneVmat_fast{16,1}=[];
...
for t=1:length(newmat)
    ...
    vecWinx5=ceil(realinx_W(t)/16);
    inx_F=realinx_W(t)-16*(vecWinx5-1);
...
if as~=0 
   curinx2vec=curinx2avec-n_vec*(inx_amat-1);
    realinx2vec=reinx.CE(curinx2vec);
    
   inx_vec5=ceil(realinx2vec/16);
   inx_E=realinx2vec-16*(inx_vec5-1);
